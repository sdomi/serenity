...
ErrorOr<void> SDHostController::initialize()
{
    ...
    if (card_or_error.is_error() && card_or_error.error().code() != ENODEV) {
        dbgln("SDHostController: Failed to initialize inserted card: {}", card_or_error.error());
...
ErrorOr<NonnullRefPtr<SDMemoryCard>> SDHostController::try_initialize_inserted_card()
{
    ...
    if (card_is_mmc) {
        // FIXME: check whether we actually support 8-bit width
        TRY(issue_command_mmc(MMC::Commands::switch_, (0b11 << 24) | (183 << 16) | (2 << 8))); // write, byte 183 (bus width), 2 (8-bit mode)
        TRY(wait_for_response());

        // This is actually a bitfield, but the standard specifies as follows:
        // "For Devices supporting versions v4.0, v4.1, and v4.2 of the standard, the value shall be 20 MHz (0x2A).
        // For Devices supporting v4.3, the value shall be 26 MHz (0x32)."
        // ... hence, there isn't much of a point of parsing the whole value.
        switch(csd.max_data_transfer_rate) {
        case 0x2a:
            TRY(sd_clock_frequency_change(20000000));
            break;
        case 0x32:
            TRY(sd_clock_frequency_change(26000000));
            break;
        }
        // TODO: HS, HS200, HS400 support
    }
...
ErrorOr<void> SDHostController::issue_command_mmc(MMC::Command const& cmd, u32 argument)
{
    ...
    // if (cmd.requires_dat_line() && cmd.type != SD::CommandType::Abort) {
    if (cmd.requires_dat_line()) {

        if (!retry_with_timeout([&]() { return !m_registers->present_state.command_inhibit_dat; })) {
            meowln("issue_command_mmc: eio on command_inhibit_dat");
            return EIO;
        }
    }
// 
...
//     case MMC::ResponseType::NoResponse:
//         flags = 0;
//         break;
//     case MMC::ResponseType::ResponseOf136Bits:
//         flags = 0x01;
//         break;
//     case MMC::ResponseType::ResponseOf48Bits:
//         flags = 0x02;
//         break;
//     case MMC::ResponseType::ResponseOf48BitsWithBusy:
//         flags = 0x03;
//     }
...
ErrorOr<SDHostController::Response> SDHostController::wait_for_response_mmc()
{
    ...
    //       reach us
    Response r = {};
    auto cmd = last_sent_command_mmc(); // TODO: refactor me
    switch (cmd.response_type) {
    case MMC::ResponseType::NoResponse:
        break;
    case MMC::ResponseType::ResponseOf136Bits:
    ...
    if (cmd.response_type != MMC::ResponseType::ResponseOf136Bits) {
        if (card_status_contains_errors_mmc(cmd, r.response[0])) {
            return EIO;
        }
    }
...
ErrorOr<SDHostController::Response> SDHostController::wait_for_response()
{
    ...
    if (!retry_with_timeout(
            [&]() { return m_registers->interrupt_status.command_complete; })) {
        meowln("EIO in wait_for_response {:hex-dump}", (u32)m_registers->interrupt_status.raw);
        return EIO;
    }
    ...
    //       reach us
    Response r = {};
    auto cmd = last_sent_command(); // TODO: refactor me
    switch (cmd.response_type) {
    ...
    case SD::ResponseType::ResponseOf48BitsWithBusy:
        r.response[0] = m_registers->response_0;
        if (!retry_with_timeout(
                [&]() { return ~m_registers->present_state.dat_line_active; })) {// is this the correct one?
            meowln("EIO in R1b: the busy did not go away :c");
            return EIO;
        }
        // TODO: send CMD_STATUS. for now we'll just wait for the busy to go away
        break;
    }
        ...
        for (; i < 32; ++i) {
            ...
            if (block_count - blocks_transferred < blocks_per_descriptor) {
                ...
                command_buffer[i] = SD::DMADescriptor128 {
                    ...
                    .address_low = static_cast<u32>((physical_transfer_address + offset) & 0xFFFFFFFF),
...
ErrorOr<SD::SDConfigurationRegister> SDHostController::retrieve_sd_configuration_register(u32 relative_card_address)
{
    ...
    TRY(transaction_control_with_data_transfer_using_the_dat_line_without_dma(
        ...
        0, 1, sizeof(SD::SDConfigurationRegister),
        UserOrKernelBuffer::for_kernel_buffer(scr.raw), DataTransferType::Read));

    return scr;
}

ErrorOr<SD::MMCExtendedCSD> SDHostController::retrieve_mmc_ext_csd(u32 relative_card_address)
{
    // TODO: linux's drivers/mmc/core/mmc_ops.c has an ominous `mmc_can_ext_csd`. it may be a good idea to check for that.
    SD::MMCExtendedCSD ext_csd;
    (void)relative_card_address; // TODO: are we sure this card is always selected?
    TRY(transaction_control_with_data_transfer_using_the_dat_line_without_dma(
        SD::Commands::mmc_send_ext_csd,
        0, 1, 512,
        UserOrKernelBuffer::for_kernel_buffer(bit_cast<u8*>(&ext_csd)), DataTransferType::Read));
...
bool SDHostController::card_status_contains_errors_mmc(MMC::Command const& command, u32 resp)
{
    (void)command;
    (void)resp;
    dbgln("card_status_contains_errors called");
    return false;
}
